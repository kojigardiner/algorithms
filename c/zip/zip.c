...
// LZW Compression
// -----------------------------
// Uses a per-file fixed-length encoding to represent sequences of bytes. The
// encoding mapping is built on-the-fly in both compression and expansion, 
// requiring only a single pass through the input file and no extra space in the
// compressed file to store the mapping table. In this implementation,
// 12-bit codes are used to represent byte sequences that appear in the file,
// and a trie is used to efficiently search the mapping table based on the 
// upcoming bytes in the file.
//
// Compression:
// First, a trie is generated with each 8-bit character value given a mapping
// to its same value in 12-bits. An extra character is added to the table to 
// represent EOF. The input file is then iterated over, searching for the
// longest prefix of the unscanned input that exists in the trie. Once a prefix
// is found, the corresponding 12-bit code is written to the output file. A new
// "lookahead" string is generated by adding the next character in the input to 
// the previously found prefix, and adding it to the trie, mapped to the next
// available code. In this way, we add progressively longer prefixes to the
// mapping table as the input file is scanned, with the hope that such prefixes
// will later show up in the input file. At the end of the file a special EOF
// code is written. If the available 12-bit codes are ever exhausted, the table
// is reset and prefixes are added anew.
// 
// Expansion: 
// First, a reverse mapping table is generated as a simple array of strings,
// where a 12-bit value indexes into the array to retrieve an arbitrary byte
// string. The compressed file is iterated over, reading 12-bit values and 
// writing their associated byte strings from the table to the output file. On
// each iteration, a lookahead operation is performed to read the next 12-bit
// code from the input file, decode it, and create a new entry in the table for
// the first byte string + first byte of the next byte string. In this way, 
// we rebuild the mapping table that was built during compression. As with
// compression, if the available 12-bit codes are ever exhausted, the table
// is reset and prefixes are added anew.
// 
// *An important note on efficiency*: because the trie and its longest_prefix_of
// function operates on C-strings, we need to take special care when
// encoding binary files with null-bytes. Because the null-byte has special
// meaning as the end-of-string null-terminator in C, this means that encoded 
// byte sequence prefixes cannot include null-bytes within them. The 
// implementations here simply write out null bytes with a 12-bit zero code when
// they are encountered. As such, there may be cases where longer common 
// prefixes exist but cannot be encoded given the C null-byte limitation, 
// resulting in less-than optimal encoding efficiency.
// 
...
// Expands the given in_filename and writes the LZW-encoded file to out_filename.
void compress_lzw(const char *in_filename, const char *out_filename) {
  // Create reader/writer
  bit_io_t *bin = bit_io_open(in_filename, "r");
  bit_io_t *bout = bit_io_open(out_filename, "w");

  // Set up a symbol table with compression codes, where the key is a sequence
  // of bytes and the value is a code of length LZW_CODE_WIDTH.
  st_t *st = reset_compress_table_lzw(NULL);

  // Keep track of used codes. LZW_EOF_CODE is the last "default" code, so one 
  // beyond that is where the rest of our per-file codes start.
  int code_counter = LZW_EOF_CODE + 1;

  // Read the entire file into memory as a string
  struct stat stat1;
  stat(in_filename, &stat1);
  char *full_file = malloc((stat1.st_size + 1) * sizeof(char)); // add 1 for final null-terminator
  if (!full_file) {
    perror("Failed to malloc\n");
    exit(EXIT_FAILURE);
  }
  ...
  // Iterate over file contents while there are still unscanned bytes. On each
  // iteration, the position pointer moves by the length of the key found.
  int keylen = 0;
  for (int position = 0; position < stat1.st_size; position += keylen) {
    // If we have maxed out our table, reset it.
    ...
    // Special case if current position is a null byte. Write a 0 byte to output
    // and move our position pointer by 1.
    ...
    // Special case if there is only one char remaining, we can skip the 
    // prefix check and just set the key to that one char string.
    ...
    // Otherwise, find the longest prefix in our table starting at the current
    // file position.
    ...
    // Update keylen to move the position pointer
    keylen = strlen(key);

    // Add a new code to the symbol table if the lookahead character isn't a 
    // null byte.
    char lookahead = full_file[position + keylen];
    if (lookahead != '\0') {
  ...
  // Write the EOF code to finish the file
...
// Expands the given LZW-encoded in_filename and writes to out_filename. 
void expand_lzw(const char *in_filename, const char *out_filename) {
  // Create reader/write
  bit_io_t *bin = bit_io_open(in_filename, "r");
  bit_io_t *bout = bit_io_open(out_filename, "w");
  ...
  // byte string to write to the output file.
  ...
  while (true) {
    ...
    // Write the current byte string to the output file.
    if (strlen(curr) != 0) {
      // Iterate over each byte and write it out.
      for (int i = 0; i < strlen(curr); i++) {
        bit_io_write_byte(bout, curr[i]);
      }
    } else {
      // Special case where the string is empty, so we write a null-byte.
      bit_io_write_byte(bout, 0);
    }

    // Read the next code, checking if we've hit EOF.
    code = read_code_lzw(bin);
    if (code == LZW_EOF_CODE) {
      break;
    }

    // Special case where the code is 0, so we set the string to empty and
    // skip the rest of the decode logic.
    if (code == 0) {
      strcpy(curr, "");
      continue;
    }
    ...
    // Decode logic
    if (table[code]) {
      // If the code exists in our table, update the next string with it
      strcpy(next, table[code]);

      // Lookahead in next, and add a new code to our table if the current 
      // string isn't empty.
      if (strlen(curr) != 0) {
        lookahead = calloc(strlen(curr) + 2, sizeof(char));
        if (!lookahead) {
          perror("Failed to calloc\n");
          exit(EXIT_FAILURE);
        }
        strcpy(lookahead, curr);

        // Add the first letter of the next string to the new codestring
        lookahead[strlen(curr)] = next[0];
        table[code_counter++] = lookahead;
      }
    } else {
...
// Reads the bits of an LZW code from the file associated with b.
int read_code_lzw(bit_io_t *b) {
  int code = 0;
  // Read MSB first
  for (int i = LZW_CODE_WIDTH - 1; i >= 0; i--) {
